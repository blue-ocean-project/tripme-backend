        ...
        for (let i = 0; i < req.body.contacts.length; i++) {
          ...
          if (utils.validateEmail(req.body.contacts[i])) {
            const info = await utils.transporter.sendMail({
              ...
              html: `<body><img src="http://titaniasgarden.com/wp-content/uploads/2021/08/TripMe.png" width="300"/><h1>You have been invited to join a trip!</h1><form action="${config.clientUrl}/invite/${req.body.trip_id}?key=${code}"><input type="submit" value="Accept Invitation"></form></body>`,
            });
